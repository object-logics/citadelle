...
a uniform term language in which all logical entities where represented:
\[
   \operatorname{term} ::= C | V | \lambda~V. \operatorname{term} | \operatorname{term}~\operatorname{term}
...
\item $C$ is the set of \emph{constant symbols}
      like $\operatorname{fst}$ or $\operatorname{snd}$ as operators on pairs. Note that
       ...
       "$(\_ \Longleftarrow \_)~A~B$" or  "$(\_ + \_)~A~B$" 
       can be parsed and printed as 
       "$A \Longleftarrow B$" or  "$A + B$", respectively.
...
\item $\lambda~V. \operatorname{term}$ is called $\lambda$-abstraction; example: $\lambda~x. x$.
      A $\lambda$-abstraction forms a scope for the variable $V$.
\item $E~E'$ is called an \emph{application}.  
\end{itemize}
...
\item $TC$ is a set of \emph{type constructors} like "$('\alpha)\operatorname{list}$" or
  ...
  $'\alpha + '\beta $ can be represented by $('\alpha,'\beta)(\_ \ < \operatorname{times} > \_)$ or 
  ...
  not that the paranthesises of null-ary type constructors are usually ommitted.
\end{itemize}.

Isabelle accepts also the notation $t :: \tau$ as type assertion in the term-language;
$t :: \tau$ means "t is required to have type $\tau$".
...
\ie{} syntactic material of a logical theory.
...
By extensions of global contexts with axioms and proofs over then,
...
constant-definition ar an axiom, Isabelle offers a number of 
\emph{commands} that allow for more complex extensions of theories in
a logically safe way.
...
$(\Forall~x\spot f~x = g~x) \Leftrightarrow f = g$.
...
On the other hand, Isabelle/\HOL can also be viewed as a 
combination of a functional programming language
like SML or Haskell. Usually, Isabelle/\HOL definitions can be 
...
the reader can just ignore these aspects in definitions.

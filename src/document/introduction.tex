...
language as example: \inlineocl{Tuples}. Recall that tuples (in other
...
\isatagafp This leads us to our second goal of this document: \endisatagafp
\isatagannexa This leads us to our second goal of this annex: \endisatagannexa
...
\begin{itemize}
 ...
 \item Wrt. to the static types,  \FOCL is a strongly typed language in
         ...
         \inlineocl{oclAsType(Class)}). \footnote{The details of such a pre-processing are
...
a uniform term language $T$ in which all logical entities where represented:
\[
   T~~ ::=~~ C~~ |~~ V ~~|~~ \lambda~V.~~ T ~~|~~ T~T
\] \footnote{In the Isabelle implementation, there are actually two further variants
which were irrelevant for this presentation and are therefore ommitted.}
where:
\begin{itemize}
...
\item $V$ is the set of \emph{variable symbols} like "$x$", "$y$", "$z$", ...
...
languages ranging from Haskell over Phyton to Java.

Terms where associated to \emph{types} by a set of \emph{type inference rules} (similar to
\url{https://en.wikipedia.org/wiki/Hindley-Milner_type_system}); only terms for which 
a type can be inferred --- \ie{} for \emph{typed terms} --- were considered as legal input
to the Isabelle system. The type-terms $\tau$ for $\lambda$-terms are defined as:
\[  \tau~~ ::=~~ TV~~ |~~ TV :: \Xi~~ |~~ \tau~\Rightarrow~\tau~~ |~~ (\tau, ..., \tau)TC\]
\footnote{Again, the Isabelle implementation is actually slightly different;
 our presentation is an abstraction in order to improve readability.}
\begin{itemize}
\item $TV$ is the set of \emph{type variables} like "$'\alpha$", "$'\beta$", ...
  Note that the syntactic categories $V$ and $TV$ are disjoint; thus,
  $'x$ is a perfectly possible type variable.
\item $\Xi$ is a set of \emph{type-classes} like \emph{ord}, \emph{order}, \emph{linorder}... 
  This feature
  in the Isabelle type system is inspired by Haskell type classes
  (cf. \url{https://en.wikipedia.org/wiki/Type_class}).
...
\item $TC$ is a set of \emph{type constructors} like "$('\alpha)\operatorname{list}$" or
  ...
  note that the paranthesises of null-ary type constructors are usually ommitted.
\end{itemize}

Isabelle accepts also the notation $t :: \tau$ as type assertion in the term-language;
$t :: \tau$ means "t is required to have type $\tau$". Note that typed terms \emph{can} 
contain free variables; terms like $x + y = y + x$ reflecting common mathematical
notation (and the convention that free variables are implicitely universally quantified)
are possible and common in Isabelle theories.\footnote{Here, we assume that $\_ + \_$ and
$\_ = \_$ are declared constant symbols having type 
$\operatorname{int} \Rightarrow \operatorname{int} \Rightarrow \operatorname{int}$ 
and
$'\alpha \Rightarrow '\alpha \Rightarrow \operatorname{bool}$, respectively}.


An environment providing $\Xi$, $TC$ as well as a map from constant symbols $C$ to types (built
over these $\Xi$ and $TC$) is called a \emph{global context}; it provides a kind of signature, 
\ie{} a mechanism to construct the syntactic material of a logical theory.

The most basic (built-in) global context of Isabelle provides just a language to construct
logical rules. More concretely, it provides a constant definition  for 
the (built-in) \emph{meta-level implication} $\_ \Implies \_$ allowing to form constructs like $A_1
...
are equal iff they are pointwise equal; this is captured by the rule:
...
with resepct to their logical aspects (they may be incomplete
or failing, though).
...
$\operatorname{sort}$ operation on linearly orderd lists by:
...
\isatagafp
\fxnote{Generate this chapter from Isabelle theories ? Just for principle ?}
\endisatagafp
...
defined as follows:
\isatagafp
 \fxnote{should we use expliucit definitions ?}
\endisatagafp
\begin{itemize}
\item $C \subseteq \text{TYPES}(C)$
\item $\text{Boolean}$, $\text{Integer}$, $\text{Real}$, $\text{Void}$, ... are elements
      of $\text{TYPES}(C)$
\item $\text{Set}(X)$, $\text{Bag}(X)$, $\text{Sequence}(X)$,  and $\text{Pair}(X,Y)$ (as example for a Tuple-type)
...
pre- and post state $(\sigma, \sigma')$. Note that we will also use $\tau$ to denota the
\emph{type} of a state-pair; since both syntactic categories are independent, we can
do so without arising confusion. \OCL provides for all \OCL types the
 constants \mocl{invalid} for the exceptional computation result and \mocl{null} for
...
\end{alignedat}
\end{gather*}
For the concrete \mocl{Boolean}-type, we define similarly the boolean constants $\mocl{true}$ and
...
\isatagafp
\fxnote{Should we add in our notion of Class-Model also the Operations ?}
\endisatagafp
...
\end{itemize}
\isatagafp
\fxwarning{correct? What should be checked? Achim: Description of Liskov is IMHO correct,
but the simulation of a vtable using oclIsKindOf is at leat tedious. One should, imho, add a vtable for modelling overridding properly (informally, we had this already in HOL-OCL)}
\endisatagafp
We do not model \emph{overriding} of operations as in Java or C++
explicitly in \FOCL.  However, it is easy expressed in this
core-language by adding \mocl{self.oclIsKindOf(C)} in the
pre-condition $\phi$ (assuming that, as in the schema above, \mocl{C}
is the context to which the contract is referring to).  In order to
avoid logical contradictions (inconsistencies) between different
instances of an overridden operation, the user has to prove Liskov's
principle for these situations: pre-conditions of the superclass must
imply pre-conditions of the subclass, and post-conditions of a
subclass must imply post-conditions of the
superclass.
%%% Local Variables:
%%% mode: latex
%%% TeX-master: "root"

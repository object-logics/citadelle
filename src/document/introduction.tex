...
context Session::findRole(person:Person):Role
  ...
  post: result=participants->one(p:Participant |
                                p.person = person ).role
        and participants = self.participants@pre
        and name = name@pre
\end{ocl}
...
logical rules. More concretely, it provides a constant declaration  for 
...
conclusion $A_{n+1}$'' and which is written in Isabelle syntax as as
\begin{gather}
...
\item $\text{Boolean}$, $\text{Integer}$, $\text{Real}$, $\text{Void}$, ... 
...
also use $\tau$ to denota the \emph{type} of a state-pair; since both syntactic 

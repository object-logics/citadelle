...
\chapter{Motivation}
  ...
  element, called \mocl{null}.  While the first exception element
  \mocl{undefined} has a strict semantics, \mocl{null} has a non
...
\subsection{A guided tour through
...
Relations between classes (called \emph{associations} in \acs{uml})
...
and \inlineocl{Role}. Associations may be labelled by a particular
...
We refine this data model using the \acf{ocl} for specifying additional
invariants, preconditions and postconditions of operations. For
example, we specify that objects of the class \inlineocl{Person} are
uniquely determined by the value of the \inlineocl{name} attribute and
that the attribute \inlineocl{name} is not equal to the empty string
(\inlineocl{''}):
\begin{ocl}
context Person
...
describing the behavior of the operation \inlineocl{findRole} as
follows:
\begin{ocl}

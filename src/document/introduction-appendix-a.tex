...
This annex formally defines the semantics of OCL and will proceed by describing 
the OCL semantics by a translation in to a core language --- called FeatherWeight OCL ---
...
in large parts executable, in some parts only provable (namely the essence of 
        Set-constructions). The main concern of its construction is \emph{consistency}, 
i.e. it should be possible to apply logical rules and evaluation rules for OCL 
in an arbitrary manner always yielding the same, except in pathological cases 
unambiguously defined result. Since OCL is a typed, object-oriented language 
based on a four-valued logics, this effort seems to be justified.
In order to make this annex usable not only by logicians, but also by developers 
of compilers and proof-tools, we  \emph{derived} from the Isabelle definitions 
also \emph{logical rules} allowing formal interactive and automated proofs on 
...
OCL is a \emph{typed} object-oriented language. This means that it is --- like 
...
\verb+OclIsNew()+, the effect of object creation can be expressed in a 
...
importance for our goal of logical consistency\footnote{it is impossible to have 
sets that contain themselves, i.e. to state Russels Paradox in OCL typed set-theory}. 
Moreover, object-oriented typing means that between types there can be sub typing
relations; technically speaking, this means that they can be \emph{casted} via 
\verb+oclIsTypeOf(T)+ one to the other, and under particular conditions to be 
...
\begin{itemize}
 ...
 \item Technically, Featherweight OCL is a \emph{semantic embedding} into a powerful semantic meta-language and
         environment, namely Isabelle/HOL~\cite{nipkow.ea:isabelle:2002}.
         It is a \emph{shallow embedding} in HOL; types

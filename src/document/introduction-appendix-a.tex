...
This annex formally defines the semantics of OCL and will proceed by describing the OCL semantics by a translation in to a core language --- called FeatherWeight OCL ---
which has in itself a formally described semantics presented in Isabelle/HOL (see \cite{} )
% http://afp.sourceforge.net/entries/Featherweight_OCL.shtml
for an up-to-date, machine-checked version 
in the Archive of Formal Proods (AFP)). The semantic definitions are 
in large parts executable, in some parts only provable (namely the essence of Set-constructions). The main concern of its construction is \emph{consistency}, i.e. it should
be possible to apply logical rules and evaluation rules for OCL in an arbitrary manner always yielding the same,
except in pathological cases unambiguously defined result. 
Since OCL is a typed, object-oriented language based on a four-valued logics, this effort seems to be justified.
In order to make this annex usable not only by logicians, but also by developers of compilers and proof-tools, we 
\emph{derived} from the Isabelle definitions also \emph{logical rules} allowing formal interactive and automated proofs on 
UML/OCL specifications, as well as \emph{execution rules} and \emph{test-cases} revealing corner-cases resulting from
this semantics which give vital information for the implementor.

OCL is a \emph{typed} object-oriented language. This means that it is --- like Java or C++ --- based on the concept of
a \emph{static type}, that is the type that the type-checker infers from a UML class model and its OCL annotation, as 
well as a dynamic type, that is the type at which an object is dynamically created. 
\footnote{As side-effect free language, OCL has no object-constructors, but with \verb+OclIsNew()+, the effect of
object creation can be expressed in a declarative way.} Types are not only a means for efficient compilation
and a support of separation of concerns in programming, there are of fundamental importance for our
goal of logical consistency\footnote{it is impossible to have sets that contain themselves, i.e. to state Russels Paradox in
OCL typed set-theory}. Moreover, object-oriented typing means that between types there can be sub typing
relations; technically speaking, this means that they can be \emph{casted} via 
\verb+oclIsTypeOf(T)+ one to the other, and under particular conditions to be described in detail later, these casts 
are semantically \emph{lossless}. 

Here is a feature-list of FeatherWeight OCL: 
\begin{enumerate}
\item It specifies key built-in types such as \verb+Boolean+, \verb+Void+, \verb+Integer+, \verb+Real+ and \verb+String+
         as well as generic types such as \verb+Pair(T,T')+, \verb+Sequence(T)+ and \verb+Set(T).
\item It defines the semantics of the operations of these types in \emph{denotational form} --- see explanation below ---, 
         and thus an unambiguous (and in Isabelle/HOL executable or animatable) way.    
\item it develops the \emph{theory} of these definitions, i.e. the collection of lemmas and theorems that
         can be proven from these definitions. In the case of the four-valued \verb+Boolean+ type, this also covers the
         derivation of the \emph{logic} of OCL.
...
\fixme{rethink this ! I think it should be the other way round !}
Sub clause A.1 defines the concept of object models. Object models provide information used as context for OCL expressions and constraints. Sub clause A.2 defines the type system of OCL and the set of standard operations. Finally, sub clause A.3 defines the semantics of OCL expressions.
\fixme{... END}


a particular organization necessary: 
\begin{enumerate}
\item all operations must first be 

lexicon: "!" "!!" "#" "$" "%" "&" "&&&" "(" "()" "(|" ")" "*" "+" "++" "," "-"
  "-->" "-`" "." ".." "..." "..<" "..}" "/" "//" "0" "1" ":" "::" ":=" ":>" ";" "<"
  "<*>" "<*lex*>" "<*mlex*>" "<+>" "<-" "<->" "<.." "<..<" "<..}" "<=" "=" "=="
  "==>" "=>" "=simp=>" ">" ">=" "?" "?!" "@" "@@" "@@@@" "ALL" "CHR" "CONST" "CSUM"
  "EX" "EX!" "GREATEST" "INF" "INFM" "INT" "Int" "LEAST" "LIM" "List.assoc"
  "List_member" "MOST" "O" "OFCLASS" "OO" "PROD" "PROP" "SIGMA" "SOME"
  "SORT_CONSTRAINT" "SUM" "SUP" "TERM" "THE" "TYPE" "TYPEREP" "UN" "Un" "WRT"
  "XCONST" "[" "[↦]" "[]" "[|" "[|->]" "⋀" "∷" "⋂" "⟹" "∏" "⇒" "⨅⇩f⇩i⇩n"
  "⨆⇩f⇩i⇩n" "∑" "⋃" "⇘" "⇙" "⇧*" "⇧*⇧*" "⇧+" "⇧+⇧+" "⇧2" "⇧=" "⇧=⇧=" "´" "∧" "¦"
  "∘" "∘⇩m" "°" "…" "≡" "∃" "∃!" "∃⇩F" "∃⇩∞" "∀" "∀⇩F" "∀⇩∞" "≥" "≫" "«|" "∈"
  "ı" "ℤ" "∩" "¯" "¯¯" "λ" "λ⇩S⇩c⇩a⇩l⇩a" "⟨" "⟦" "≤" "←" "≪" "⟷" "⟶" "⦇" "↦"
  "ℕ" "¬" "≠" "∉" "∨" "≺" "⟩" "⟧" "⇀" "⦈" "ϵ" "⋄" "⊂" "⊆" "⊆⇩m" "≻" "⊃" "⊇" "×"
  "∪" "]" "^" "^*" "^**" "^+" "^++" "^--1" "^-1" "^=" "^==" "^^" "_" "`" "``"
  "case" "case⇩O⇩C⇩a⇩m⇩l" "case⇩S⇩c⇩a⇩l⇩a" "chain⇩⊆" "div" "dvd" "else" "entries"
  "fold" "if" "in" "initial_segment_of" "insert" "insert2" "keys" "let"
  "let⇩O⇩C⇩a⇩m⇩l" "lookup" "lookup2" "map_entry" "mod" "modify_def" "o" "o_m" "of"
  "op" "respects" "respects2" "sprint0" "sprint1" "sprint2" "sprint3" "sprint4"
  "sprint5" "then" "{" "{.." "{..<" "{}" "|" "|)" "|->" "|«" "|]" "|`" "}" "~" "~:"
  "~=" "~=>"
prods:
  ...
  logic = "op" "|«" => "\<^const>RBT_Impl.ord_class.rbt_less_symbol" (1000)
  logic = "op" "«|" => "\<^const>RBT_Impl.ord_class.rbt_greater" (1000)
  ...
  logic = "⟨" logic[0] "⟩" => "_string1" (1000)
  logic = "≺" logic[0] "≻" => "_string2" (1000)
  logic = "≪" logic[0] "≫" => "_string3" (1000)
  logic = "°" logic[0] "°" => "_char1" (1000)
  logic = "List.assoc" => "_list_assoc" (1000)
  logic = "List_member" => "_list_member" (1000)
  logic = "let⇩O⇩C⇩a⇩m⇩l" HOL.letbinds[0] "in" any[10] => "_Let⇩O⇩C⇩a⇩m⇩l" (10)
  logic = "case⇩O⇩C⇩a⇩m⇩l" any[0] "of" HOL.cases_syn[0] => "_case_syntax⇩O⇩C⇩a⇩m⇩l"
    (10)
  logic = "λ⇩S⇩c⇩a⇩l⇩a" pttrn[0] pttrn[0] "." logic[10] => "_Lambda⇩S⇩c⇩a⇩l⇩a" (10)
  logic = "λ⇩S⇩c⇩a⇩l⇩a" pttrn[0] "." logic[10] => "_Lambda⇩S⇩c⇩a⇩l⇩a" (10)
  logic = "case⇩S⇩c⇩a⇩l⇩a" any[0] "of" HOL.cases_syn[0] => "_case_syntax⇩S⇩c⇩a⇩l⇩a"
    (10)
  ...
  logic = "sprint0" logic[0] "´" => "_sprint0" (1000)
  logic = "sprint1" logic[0] "´" => "_sprint1" (1000)
  logic = "sprint2" logic[0] "´" => "_sprint2" (1000)
  logic = "sprint3" logic[0] "´" => "_sprint3" (1000)
  logic = "sprint4" logic[0] "´" => "_sprint4" (1000)
  logic = "sprint5" logic[0] "´" => "_sprint5" (1000)
  logic = logic[51] "|«" any[51] => "\<^const>RBT_Impl.ord_class.rbt_less_symbol"
    (50)
  ...
  logic = logic[4] "::" type[0] => "_constrain" (3)
  logic = any[51] "«|" logic[51] => "\<^const>RBT_Impl.ord_class.rbt_greater" (50)
...
consts: "\<^const>Filter.Alm_all_binder" "\<^const>Filter.Inf_many_binder"
  ...
  "_Lambda⇩S⇩c⇩a⇩l⇩a" "_LeastM" "_Let" "_Let⇩O⇩C⇩a⇩m⇩l" "_Map" "_MapUpd" "_Maplets"
  "_Numeral" "_SUP" "_SUP1" "_Setcompr" "_Sigma" "_String" "_TYPE" "_TYPEREP"
  "_The" "_UNION" "_UNION1" "_UNION_le" "_UNION_less" "_Update" "_abs" "_applC"
  "_aprop" "_args" "_asm" "_asms" "_bigimpl" "_bind" "_binds" "_bound" "_bracket"
  "_cargs" "_cartouche_string" "_case1" "_case2" "_case_syntax"
  "_case_syntax⇩O⇩C⇩a⇩m⇩l" "_case_syntax⇩S⇩c⇩a⇩l⇩a" "_char1" "_class_name"
  "_classes" "_constify" "_constrain" "_constrainAbs" "_context_const"
  "_context_xconst" "_dummy_ofsort" "_eventually" "_field" "_field_type"
  "_field_types" "_field_update" "_field_updates" "_fields" "_filter" "_free"
  "_frequently" "_from_to_setprod" "_from_to_setsum" "_from_upto_setprod"
  "_from_upto_setsum" "_idtdummy" "_idts" "_idtyp" "_idtypdummy" "_ignore_type"
  "_index" "_indexdefault" "_indexvar" "_inner_string" "_lam_pats_syntax" "_lambda"
  "_lc_abs" "_lc_end" "_lc_gen" "_lc_quals" "_lc_test" "_list" "_list_assoc"
  "_list_member" "_listcompr" "_listprod" "_listsum" "_loose" "_lupdbind"
  "_lupdbinds" "_maplet" "_maplets" "_mk_ofclass" "_numeral" "_ofclass" "_ofsort"
  "_pattern" "_patterns" "_position" "_position_sort" "_pttrns" "_qsetprod"
  "_qsetsum" "_rbt_entries" "_rbt_fold" "_rbt_insert" "_rbt_insert2" "_rbt_keys"
  "_rbt_lookup" "_rbt_lookup2" "_rbt_map_entry" "_rbt_modify_def" "_record"
  "_record_scheme" "_record_type" "_record_type_scheme" "_record_update"
  "_setleAll" "_setleEx" "_setleEx1" "_setlessAll" "_setlessEx" "_setprod"
  "_setsum" "_sort" "_sort_constraint" "_sprint0" "_sprint1" "_sprint2" "_sprint3"
  "_sprint4" "_sprint5" "_string1" "_string2" "_string3" "_strip_positions"
  "_struct" "_tapp" "_tappl" "_tfree" "_topsort" "_tuple" "_tuple_arg"
  "_tuple_args" "_tvar" "_type_constraint_" "_type_name" "_type_prop" "_types"
  "_update_name" "_updbind" "_updbinds" "_upt_setprod" "_upt_setsum"
  "_upto_setprod" "_upto_setsum" "_var"
...
parse_rules:
    ...
    ("_char1" x)  ->
      ("\<^const>OCL_compiler_init.abr_string.SS_base"
        ("\<^const>OCL_compiler_init.string⇩b⇩a⇩s⇩e.ST'"
          ("\<^const>List.list.Cons" x "\<^const>List.list.Nil")))
    ...
    ("_sprint0" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf0" x)
    ("_sprint1" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf1" x)
    ("_sprint2" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf2" x)
    ("_sprint3" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf3" x)
    ("_sprint4" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf4" x)
    ("_sprint5" x)  ->  ("\<^const>OCL_compiler_printer_init.sprintf5" x)
    ("_string1" x)  ->
      ("\<^const>OCL_compiler_init.abr_string.SS_base"
        ("\<^const>OCL_compiler_init.string⇩b⇩a⇩s⇩e.ST" ("\<^const>String.STR" x)))
    ("_string2" x)  ->  ("\<^const>String.STR" x)
    ("_string3" x)  ->
      ("\<^const>OCL_compiler_init.abr_string.SS_base"
        ("\<^const>OCL_compiler_init.string⇩b⇩a⇩s⇩e.ST'" x))
    ("_All_less" x y P)  ->
    ...
    ("_qsetprod" x P t)  ->
      ("\<^const>Groups_Big.comm_monoid_mult_class.setprod" ("_abs" x t)
        ("_Coll" x P))
    "_rbt_fold"  ->  "\<^const>OCL_compiler_meta_UML.fold'"
    "_rbt_keys"  ->  "\<^const>OCL_compiler_meta_UML.keys'"
    ("_setleAll" A B P)  ->
    ...
    "_list_assoc"  ->  "\<^const>OCL_compiler_init.List_assoc'"
    "_rbt_insert"  ->  "\<^const>OCL_compiler_meta_UML.insert'"
    "_rbt_lookup"  ->  "\<^const>OCL_compiler_meta_UML.lookup'"
    ("_setlessAll" A B P)  ->
      ("\<^const>HOL.All_binder" A
    ...
    "_list_member"  ->  "\<^const>OCL_compiler_init.List_member'"
    "_rbt_entries"  ->  "\<^const>OCL_compiler_meta_UML.entries'"
    "_rbt_insert2"  ->  "\<^const>OCL_compiler_meta_UML.insert2'"
    "_rbt_lookup2"  ->  "\<^const>OCL_compiler_meta_UML.lookup2'"
    ("_upt_setprod" i n t)  ->
    ...
    "_rbt_map_entry"  ->  "\<^const>OCL_compiler_meta_UML.map_entry'"
    ...
    "_rbt_modify_def"  ->  "\<^const>OCL_compiler_meta_UML.modify_def'"
    ...
    ("_from_upto_setprod" x a b t)  ->
      ("\<^const>Groups_Big.comm_monoid_mult_class.setprod" ("_abs" x t)
        ("\<^const>Set_Interval.ord_class.atLeastLessThan" a b))
    ("_Let⇩O⇩C⇩a⇩m⇩l" ("_binds" b bs) e)  ->  ("_Let⇩O⇩C⇩a⇩m⇩l" b ("_Let" bs e))
    ("_Let⇩O⇩C⇩a⇩m⇩l" ("_bind" x a) e)  ->
      ("\<^const>Fun.id" ("\<^const>HOL.Let" a ("_abs" x e)))
    ("_Lambda⇩S⇩c⇩a⇩l⇩a" x y P)  ->  ("\<^const>Fun.id" ("_abs" x ("_abs" y P)))
    ("_Lambda⇩S⇩c⇩a⇩l⇩a" x P)  ->  ("\<^const>Fun.id" ("_abs" x P))
    ("_case_syntax⇩O⇩C⇩a⇩m⇩l" v ("_case1" w x))  ->
      ("\<^const>Fun.id" ("_case_syntax" v ("_case1" w x)))
    ("_case_syntax⇩O⇩C⇩a⇩m⇩l" v ("_case2" ("_case1" w x) ("_case1" y z)))  ->
      ("\<^const>Fun.id"
        ("_case_syntax" v ("_case2" ("_case1" w x) ("_case1" y z))))
    ("_case_syntax⇩S⇩c⇩a⇩l⇩a" v ("_case1" w x))  ->
      ("\<^const>Fun.id" ("_case_syntax" v ("_case1" w x)))
    ("_case_syntax⇩S⇩c⇩a⇩l⇩a" v ("_case2" ("_case1" w x) ("_case1" y z)))  ->
      ("\<^const>Fun.id"
        ("_case_syntax" v ("_case2" ("_case1" w x) ("_case1" y z))))
parse_translation: "\<^const>Filter.Alm_all_binder"
  ...
  "_Numeral" "_Setcompr" "_TYPE" "_TYPEREP" "_abs" "_aprop" "_cartouche_string"
  "_case_syntax" "_index" "_lam_pats_syntax" "_listcompr" "_ofclass" "_record"
  "_record_scheme" "_record_type" "_record_type_scheme" "_record_update"
  "_sort_constraint" "_struct" "_update_name"
print_translation: "\<^const>Complete_Lattices.INTER"
  ...
  "\<^const>OCL_compiler_meta_Isabelle.hol_thy_locale.HolThyLocale_header_update"
  "\<^const>OCL_compiler_meta_Isabelle.hol_thy_locale.HolThyLocale_name_update"
  "\<^const>OCL_compiler_meta_Isabelle.hol_thy_locale.hol_thy_locale_ext"
  "\<^const>OCL_compiler_meta_Isabelle.hol_thy_locale.more_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_accessor_rbt_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_class_spec_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_design_analysis_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_disable_thy_output_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_file_out_path_dep_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_generation_syntax_shallow_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_higher_order_ty_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_import_compiler_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_instance_rbt_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_ocl_env_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_oid_start_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_output_position_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_sorry_dirty_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.D_state_rbt_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.more_update"
  "\<^const>OCL_compiler_meta_META.ocl_compiler_config.ocl_compiler_config_ext"
  "\<^const>OCL_compiler_meta_UML.inheritance.Inh_sib_unflat_update"
  "\<^const>OCL_compiler_meta_UML.inheritance.Inh_sib_update"
  "\<^const>OCL_compiler_meta_UML.inheritance.Inh_update"
  "\<^const>OCL_compiler_meta_UML.inheritance.inheritance_ext"
  "\<^const>OCL_compiler_meta_UML.inheritance.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_association.OclAss_relation_update"
  "\<^const>OCL_compiler_meta_UML.ocl_association.OclAss_type_update"
  "\<^const>OCL_compiler_meta_UML.ocl_association.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_association.ocl_association_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.ClassRaw_abstract_update"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.ClassRaw_clause_update"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.ClassRaw_name_update"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.ClassRaw_own_update"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_class_raw.ocl_class_raw_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt.Ctxt_clause_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt.Ctxt_param_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt.Ctxt_ty_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt.ocl_ctxt_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt_pre_post.Ctxt_expr_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt_pre_post.Ctxt_fun_name_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt_pre_post.Ctxt_fun_ty_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt_pre_post.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ctxt_pre_post.ocl_ctxt_pre_post_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_multiplicity.TyCollect_update"
  "\<^const>OCL_compiler_meta_UML.ocl_multiplicity.TyMult_update"
  "\<^const>OCL_compiler_meta_UML.ocl_multiplicity.TyRole_update"
  "\<^const>OCL_compiler_meta_UML.ocl_multiplicity.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_multiplicity.ocl_multiplicity_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.TyObj_ass_arity_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.TyObj_ass_id_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.TyObj_from_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.TyObj_name_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.TyObj_to_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class.ocl_ty_class_ext"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class_node.TyObjN_ass_switch_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class_node.TyObjN_role_multip_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class_node.TyObjN_role_ty_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class_node.more_update"
  "\<^const>OCL_compiler_meta_UML.ocl_ty_class_node.ocl_ty_class_node_ext"
  "\<^const>OCL_compiler_meta_UML_extended.ocl_instance_single.Inst_attr_update"
  "\<^const>OCL_compiler_meta_UML_extended.ocl_instance_single.Inst_name_update"
  "\<^const>OCL_compiler_meta_UML_extended.ocl_instance_single.Inst_ty_update"
  "\<^const>OCL_compiler_meta_UML_extended.ocl_instance_single.more_update"
  "\<^const>OCL_compiler_meta_UML_extended.ocl_instance_single.ocl_instance_single_ext"
  ...
  "\<^type>OCL_compiler_meta_Isabelle.hol_thy_locale.hol_thy_locale_ext"
  "\<^type>OCL_compiler_meta_META.ocl_compiler_config.ocl_compiler_config_ext"
  "\<^type>OCL_compiler_meta_UML.inheritance.inheritance_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_association.ocl_association_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_class_raw.ocl_class_raw_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_ctxt.ocl_ctxt_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_ctxt_pre_post.ocl_ctxt_pre_post_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_multiplicity.ocl_multiplicity_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_ty_class.ocl_ty_class_ext"
  "\<^type>OCL_compiler_meta_UML.ocl_ty_class_node.ocl_ty_class_node_ext"
  "\<^type>OCL_compiler_meta_UML_extended.ocl_instance_single.ocl_instance_single_ext"
...
print_rules:
    ...
    ("\<^const>Fun.id"
      ("_case_syntax" v ("_case2" ("_case1" w x) ("_case1" y z))))  ->
      ("_case_syntax⇩S⇩c⇩a⇩l⇩a" v ("_case2" ("_case1" w x) ("_case1" y z)))
    ("\<^const>Fun.id" ("_abs" x ("_abs" y P)))  ->  ("_Lambda⇩S⇩c⇩a⇩l⇩a" x y P)
    ("\<^const>Fun.id" ("_abs" x P))  ->  ("_Lambda⇩S⇩c⇩a⇩l⇩a" x P)
    ("\<^const>Fun.id" ("_case_syntax" v ("_case1" w x)))  ->
      ("_case_syntax⇩O⇩C⇩a⇩m⇩l" v ("_case1" w x))
    ("\<^const>Fun.id"
      ("_case_syntax" v ("_case2" ("_case1" w x) ("_case1" y z))))  ->
    ...
    "\<^const>OCL_compiler_meta_UML.fold'"  ->  "_rbt_fold"
    "\<^const>OCL_compiler_meta_UML.keys'"  ->  "_rbt_keys"
    ("\<^const>Product_Type.prod.case_prod"
      ("_abs" x ("_abs" ("_pattern" y zs) b)))  ->
      ("_abs" ("_pattern" x ("_patterns" y zs)) b)
    ("\<^const>Product_Type.prod.case_prod" ("_abs" x ("_abs" y b)))  ->
    ...
    "\<^const>OCL_compiler_init.List_assoc'"  ->  "_list_assoc"
    "\<^const>OCL_compiler_meta_UML.insert'"  ->  "_rbt_insert"
    "\<^const>OCL_compiler_meta_UML.lookup'"  ->  "_rbt_lookup"
    "\<^const>OCL_compiler_init.List_member'"  ->  "_list_member"
    "\<^const>OCL_compiler_meta_UML.entries'"  ->  "_rbt_entries"
    "\<^const>OCL_compiler_meta_UML.insert2'"  ->  "_rbt_insert2"
    "\<^const>OCL_compiler_meta_UML.lookup2'"  ->  "_rbt_lookup2"
    "\<^const>OCL_compiler_meta_UML.map_entry'"  ->  "_rbt_map_entry"
    "\<^const>OCL_compiler_meta_UML.modify_def'"  ->  "_rbt_modify_def"
    ("\<^const>OCL_compiler_printer_init.sprintf0" x)  ->  ("_sprint0" x)
    ("\<^const>OCL_compiler_printer_init.sprintf1" x)  ->  ("_sprint1" x)
    ("\<^const>OCL_compiler_printer_init.sprintf2" x)  ->  ("_sprint2" x)
    ("\<^const>OCL_compiler_printer_init.sprintf3" x)  ->  ("_sprint3" x)
    ("\<^const>OCL_compiler_printer_init.sprintf4" x)  ->  ("_sprint4" x)
    ("\<^const>OCL_compiler_printer_init.sprintf5" x)  ->  ("_sprint5" x)
    ("\<^const>Complete_Lattices.Inf_class.INFIMUM" "\<^const>Set.UNIV"
      ("_abs" x B))  ->
      ("_INF1" x B)
    ("\<^const>Complete_Lattices.Inf_class.INFIMUM" A ("_abs" x B))  ->
      ("_INF" x A B)
    ("_Let⇩O⇩C⇩a⇩m⇩l" b ("_Let" bs e))  ->  ("_Let⇩O⇩C⇩a⇩m⇩l" ("_binds" b bs) e)
    ("\<^const>Complete_Lattices.Sup_class.SUPREMUM" "\<^const>Set.UNIV"
      ("_abs" x B))  ->
      ("_SUP1" x B)
    ("\<^const>Complete_Lattices.Sup_class.SUPREMUM" A ("_abs" x B))  ->

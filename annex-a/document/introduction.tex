...
\begin{gather}
 \getAttrib{\mocl{invalid}}{\text{a}} = \mocl{invalid}\\
 \getAttrib{\mocl{null}}{\text{a}} = \mocl{invalid} \\
 \getAttrib{\mocl{invalid}}{\text{a}\isasymOclATpre} = \mocl{invalid}\\
 \getAttrib{\mocl{null}}{\text{a}\isasymOclATpre} = \mocl{invalid} \\
 \getRole{\mocl{invalid}}{\text{r}} = \mocl{invalid}\\
 \getRole{\mocl{null}}{\text{r}} = \mocl{invalid} \\
 \getRole{\mocl{invalid}}{\text{r}\isasymOclATpre} = \mocl{invalid}\\
...
\fixme{Should we add in our notion of Class-Model also the Operations ?}
...
\item $\self$ must be defined, and the arguments valid in $\tau$: \\
      $\isasymMathOclValid \isasymMathOclIsDefined~elf \land \tau \isasymMathOclValid \isasymupsilon~x_1 \land \ldots \land  \tau \isasymMathOclValid \isasymupsilon~ x_n$
...
\begin{gather}
  \begin{prooftree}
  \tau \isasymMathOclValid \delta~ self \qquad \tau \isasymMathOclValid \phi~self~a_1\ldots a_n
  \justifies
  (\tau \isasymMathOclValid E(f_{op}~self~a_1\ldots a_n)) = 
...
\item the postcondition $\psi~self~a_1~\ldots~a_n~result$ must be decomposable
      into a $\psi'~self~a_1~\ldots a_n$ and $result = BODY~self~a_1~\ldots~a_n$.
\end{itemize}
      
We do not model overloading of operations explicitly in \FOCL. However, it is easy expressed in 
this core-language by adding \mocl{self.oclIsKindOf(C)} in the pre-condition $\phi$ (assuming
  that, as in the schema above, \mocl{C} is the context to which the contract is referring to).
In order to avoid contradictions of 

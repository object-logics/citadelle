...
defined as follows: \fixme{should we use expliucit definitions ?}
\begin{itemize}
...
\fixme{why does backslash null not work here ?}
...
\begin{gather*}
  ...
  \end{array}
\end{gather*}
\fixme{we must uniformize the list - vs. lfloor notation. Either the one or the other.}
\begin{gather*}
  \begin{array}{ll}
...
There are cases where stricness is handled differently: For example, since \mocl{Set}'s may contain 
the \mocl{null}-element, it is necessary to allow  \mocl{null} as argument for 
\mocl{_->including()}:
\begin{gather*}
   \begin{array}{rl}
...
% Abs_Set\<^sub>b\<^sub>a\<^sub>s\<^sub>e \<lfloor>\<lfloor> \<lceil>\<lceil>Rep_Set\<^sub>b\<^sub>a\<^sub>s\<^sub>e (x \<tau>)\<rceil>\<rceil>  \<union> {y \<tau>} \<rfloor>\<rfloor>
...
\begin{multlined}
 ...
 (\tau \models \mocl{not} (\upsilon x)) = (\lnot \tau \models \upsilon x) % (*  UML_Logic.foundation7':*)
\end{multlined}
\end{gather*}
...
\subsection{Denotational Semantics of Accessors on Objects and Associations}
...
\fixme{generalize OclAny and Person away  ... by Ci and Cj and Ci < Cj ....}
\begin{ocl}
 \<tau> \<Turnstile> X .oclAsType(OclAny) \<triangleq> X
 \<tau> \<Turnstile> invalid .oclAsType(OclAny) \<triangleq> invalid
 \<tau> \<Turnstile> null .oclAsType(OclAny) \<triangleq> null
 \<tau> \<Turnstile> ((X::Person) .oclAsType(OclAny) .oclAsType(Person) \<triangleq> X)
 \<tau> \<Turnstile> X .oclAsType(OclAny) .oclAsType(Person) \<triangleq> X
 \<tau> \<Turnstile> \<upsilon> (X :: Person) \<Longrightarrow> \<tau> \<Turnstile> (X .oclIsTypeOf(Person) implies (X .oclAsType(OclAny) .oclAsType(Person)) \<doteq> X)"
 \<tau> \<Turnstile> X .oclAsType(OclAny) \<triangleq> X
 \<tau> \<Turnstile> invalid .oclAsType(OclAny) \<triangleq> invalid
 \<tau> \<Turnstile> null .oclAsType(OclAny) \<triangleq> null
 \<tau> \<Turnstile> ((X::Person) .oclAsType(OclAny) .oclAsType(Person) \<triangleq> X)
 \<tau> \<Turnstile> X .oclAsType(OclAny) .oclAsType(Person) \<triangleq> X
 \<tau> \<Turnstile> \<upsilon> (X :: Person) \<Longrightarrow> \<tau> \<Turnstile> (X .oclIsTypeOf(Person) implies (X .oclAsType(OclAny) .oclAsType(Person)) \<doteq> X)"
 \<tau> \<Turnstile> \<delta> X \<Longrightarrow> \<tau> \<Turnstile> X .oclAsType(OclAny) .oclAsType(Person) \<triangleq> X
 \<tau> \<Turnstile> \<upsilon> X \<Longrightarrow> \<tau> \<Turnstile> X .oclIsTypeOf(Person) implies X .oclAsType(OclAny) .oclAsType(Person) \<doteq> X
 \<tau> \<Turnstile> X .oclIsTypeOf(OclAny) \<Longrightarrow> \<tau> \<Turnstile> \<delta> X \<Longrightarrow> \<tau> \<Turnstile> not (\<upsilon> X .oclAsType(Person))
 \<tau> \<Turnstile> invalid .oclIsTypeOf(Person) \<triangleq> invalid
 \<tau> \<Turnstile> null .oclIsTypeOf(Person) \<triangleq> true
 ...
 (\<tau> \<Turnstile> X \<doteq> X) = (\<tau> \<Turnstile> if \<upsilon> X then true else invalid endif)
 \<tau> \<Turnstile> X \<doteq> Y \<Longrightarrow>  \<tau> \<Turnstile> Y \<doteq> X 
 \<tau> \<Turnstile> X \<doteq> Y \<Longrightarrow>  \<tau> \<Turnstile> Y \<doteq> Z  \<Longrightarrow>  \<tau> \<Turnstile> X \<doteq> Z 
\end{ocl}

\subsection{Algebraic Properties of the Class-Models}\label{sec:algprop-datamodel}
In this section, we assume to be $C_i \in C$ and  $C_i < C_j$. 
The operations induced from a class-model have the following  properties:
\begin{ocl}
 invalid .oclIsTypeOf(C_i) = invalid               
 null .oclIsTypeOf(C_i) = true                     
 invalid .oclIsKindOf(C_i) = invalid               
 null .oclIsKindOf(C_i) = true                     
 (X::C_i) .oclAsType(C_i) = X                    
 invalid .oclAsType(C_i) = invalid              
 null .oclAsType(C_i) = null                      
 (X::C_i) \<doteq> X = if \<upsilon> X then true else invalid endif 

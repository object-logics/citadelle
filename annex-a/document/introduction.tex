...
the OCL semantics by a translation into a core language --- called FeatherWeight OCL ---
...
\begin{itemize}
 ...
 \item all types in FeatherWeight OCL contain the elements \inlineocl{null} and \inlineocl{invalid};        
         since this extends to \inlineocl+Boolean+ type, this results 
         in a four-valued logic. Consequently, FeatherWeight OCL contains
 ...
 \item collection types may contain
         ...
         \inlineocl{invalid}).
 \item Wrt. to the static types, Featherweight OCL a strongly typed language in 
 ...
 \item As part of the OCL logic, Featherweight OCL develops the theory of
         ...
         OCL terms.
 \item Technically, Featherweight OCL is a \emph{semantic embedding} into a 
...
notations, we present the formal semantics of FeatherWeight OCL introducing:
\begin{enumerate}
...
\label{sec:invlogic}
(*
The \OCL semantics  uses different interpretation
functions for invariants and pre-conditions; instead, we achieve their semantic
effect by a syntactic transformation $\__\text{pre}$ which replaces all accessor
functions $\getAttrib{\_}{i}$ by their counterparts
$\getAttrib{\_}{i\isasymOclATpre}$. For example, $(\getAttrib{\self}{i} >
5)_\text{pre}$ is just $(\getAttrib{\self}{i\isasymOclATpre} > 5)$.  The
operation $\_\isasymOclAllInstances$ is also substituted by its
$\isasymOclATpre$ counterpart.  Thus, we can re-formulate the semantics of the
two \OCL top-level constructs, invariant specification and method specification,
as follows:
\begin{gather}\label{eq:inv}
\begin{aligned}
& I\semantics{\mathtt{context}~c:C~\mathtt{inv}~n: \phi(c)} \tau \equiv  \\
&\qquad \tau  \isasymMathOclValid  (C\isasymOclAllInstances\isasymOclForAll(x
\text{|} \phi(x)))~\land \\
&\qquad \tau  \isasymMathOclValid (C\isasymOclAllInstances\isasymOclForAll(x
\text{|} \phi(x)))_\text{pre} 
\end{aligned}
\end{gather}
The standard forbids expressions containing $\isasymOclATpre$ constructs in
invariants or preconditions syntactically; thus, mixed forms cannot arise.
Since operations have strict semantics in \OCL, we have to distinguish for a specification of an
$\mathit{op}$ with the arguments $a_1$, \ldots, $a_n$ the
two cases where all arguments are defined (and $\self$ is non-null), or not. In
the former case, a method call can be replaced by a $\mathit{result}$
that satisfies the contract, in the latter case the argument is
$\isasymMathOclUndefined$:
...
\begin{aligned}
 I\semantics{& \mathtt{context}~C~:: \mathit{op}(a_1, \ldots, a_n) : T \\
                     ...
                     & \qquad\mathtt{post}~\psi(\self, a_1, \ldots, a_n, \mathit{result})} \tau \equiv  
 \forall s,  x_1, \ldots, x_n.  \\
 & \qquad\qquad\qquad\qquad\quad
 ~ ~  \Delta( s, x_1, \ldots, x_n) \land \tau  \isasymMathOclValid \phi(s, x_1, \ldots, x_n)_\text{pre} \\
& \qquad\qquad\qquad\qquad
\qquad  \quad \implies \tau \isasymMathOclValid \psi(s, x_1, \ldots, x_n, s.\mathit{op}(x_1, \ldots, x_n))\\
&\qquad\qquad\qquad\qquad
~ ~ ~ \land \lnot \Delta( s, x_1, \ldots, x_n) \implies  \tau  \isasymMathOclValid   s.\mathit{op}(x_1, \ldots, x_n) \isasymMathOclStrongEq \isasymMathOclUndefined
\end{aligned}
\end{gather}
where $\Delta( s, x_1, \ldots, x_n)$ is an abbreviation for $\tau
\isasymMathOclValid s \isasymMathOclStrongNotEq \Null{} \land \tau
\isasymMathOclValid \isasymMathOclIsDefined s \land \tau \isasymMathOclValid
\isasymMathOclIsDefined x_1 \land \ldots \tau \isasymMathOclValid
\isasymMathOclIsDefined x_n$. This definition captures the two cases: if the
arguments of an operation are defined and, moreover, $\self$ is not $\Null{}$,
the result of a method call must satisfy the specification; otherwise the
operation will be strict and return invalid $\isasymMathOclUndefined$.  By these
definitions an \OCL specification, \ie, a sequence of invariant declarations and
operation contracts, can be transformed into a set of (logically conjoined)
statements which is called the \emph{context} $\Gamma_{\tau}$. The \emph{theory}
of an \OCL specification is the set of all valid transitions $\tau
\isasymMathOclValid \phi$ that can be derived from $\Gamma_{\tau}$. For the
logical connectives of \OCL, a conventional Gentzen-style calculus for pairs of
the form $\Gamma_{\tau} \vdash \phi$ can be developed that allows for inferring
valid transitions from $\Gamma_{\tau}$ by deduction
(cf.~\cite{brucker.ea:semantics:2009}).  Due to the inclusion of arithmetic, any
calculus for \OCL is necessarily incomplete. It is straight-forward to extend
our notion of context to multi-transition contexts such as:
\begin{gather*}
  \Gamma \equiv \bigl\{(\sigma, \sigma')  \isasymMathOclValid \phi,
(\sigma', \sigma'')  \isasymMathOclValid \psi \bigr\}
\end{gather*}

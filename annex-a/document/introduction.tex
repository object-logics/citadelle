...
\begin{gather}
  \begin{prooftree}
    \mocl{\<And>  \<tau> \<Turnstile> \<psi> self a1 ... an res \<Longrightarrow> \<tau> \<Turnstile> E res}
    \justifies
     \mocl{\<tau> \<Turnstile> E (f self a1 ... an)}
    \end{prooftree}
\end{gather}
...
\item the post-condition must be satisfiable (``the operation must be implementable''):
      $\exists res. \tau \models \psi ~self ~a1 \ldots an~res $.
\end{itemize}
...
\begin{gather}
  \begin{prooftree}
  \mocl{\<tau> \<Turnstile> \<delta> self} \qquad \mocl{\<tau> \<Turnstile> \<phi> self a1 ... an}
  \justifies
  \mocl{(\<tau> \<Turnstile> E (f self a1 ... an)) = (\<tau> \<Turnstile> E (BODY self a1 ... an))}   
  \end{prooftree}
\end{gather}
where  
\begin{itemize}
